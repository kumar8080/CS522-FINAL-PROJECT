...
def getTitleExperienceVector(title):
	...
	if year is None: return vector
	year =  int(year.group(1))
	countPeople = 0
	if title in movieToCrew:
		crew = movieToCrew[title]
		for person in crew:
			if (person[1]==actors or person[1]==actresses) and person[0] in peopleToExperience:
				if len(peopleToExperience[person[0]])<20: continue
			countPeople+=1
			vector+=(getPersonVectorExperience(person[0],year))
	vector=vector/countPeople
	return vector
